...
Internals and structure of the Zod library](/packages/zod)[Zod Core

Zod Core package - minimal core functionality for custom implementations](/packages/core)

### On this page

[Tree-shaking](#tree-shaking)[When (not) to use Zod Mini](#when-not-to-use-zod-mini)[DX](#dx)[Backend development](#backend-development)[Internet speed](#internet-speed)[`ZodMiniType`](#zodminitype)[`.parse`](#parse)[`.check()`](#check)[`.register()`](#register)[`.brand()`](#brand)[`.clone(def)`](#clonedef)[No default locale](#no-default-locale)