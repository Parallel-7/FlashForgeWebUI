...
Complete changelog and migration guide for upgrading from Zod 3 to Zod 4](/v4/changelog)

### On this page

[Versioning](#versioning)[Why a new major version?](#why-a-new-major-version)[Benchmarks](#benchmarks)[14x faster string parsing](#14x-faster-string-parsing)[7x faster array parsing](#7x-faster-array-parsing)[6.5x faster object parsing](#65x-faster-object-parsing)[100x reduction in `tsc` instantiations](#100x-reduction-in-tsc-instantiations)[2x reduction in core bundle size](#2x-reduction-in-core-bundle-size)[Introducing Zod Mini](#introducing-zod-mini)[6.6x reduction in core bundle size](#66x-reduction-in-core-bundle-size)[Metadata](#metadata)[The global registry](#the-global-registry)[`.meta()`](#meta)[JSON Schema conversion](#json-schema-conversion)[Recursive objects](#recursive-objects)[File schemas](#file-schemas)[Internationalization](#internationalization)[Error pretty-printing](#error-pretty-printing)[Top-level string formats](#top-level-string-formats)[Custom email regex](#custom-email-regex)[Template literal types](#template-literal-types)[Number formats](#number-formats)[Stringbool](#stringbool)[Simplified error customization](#simplified-error-customization)[Upgraded `z.discriminatedUnion()`](#upgraded-zdiscriminatedunion)[Multiple values in `z.literal()`](#multiple-values-in-zliteral)[Refinements live inside schemas](#refinements-live-inside-schemas)[`.overwrite()`](#overwrite)[An extensible foundation: `zod/v4/core`](#an-extensible-foundation-zodv4core)[Wrapping up](#wrapping-up)