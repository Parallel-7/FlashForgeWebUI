 ...
 *
 * @module services/calibration/report/ReportRenderer
 */

import { PDFDocument, rgb, StandardFonts } from 'pdf-lib';
import { PNG } from 'pngjs';
import type { AnalysisResult, CalibrationSettings, MeshData, WorkflowData } from '../../../types/calibration';
...
export async function renderReportPDF(options: ReportPDFOptions): Promise<Buffer> {
  ...
  if (summaryLines.length > 0) {
    ...
    for (const line of summaryLines) {
      page.drawText(`â€¢ ${line}`, { x: margin + 4, y: cursorY, size: 11, font, color: rgb(0.25, 0.25, 0.25) });
      cursorY -= 14;
    }
    cursorY -= 6;
  }
