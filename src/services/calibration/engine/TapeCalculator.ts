 ...
 *
 * @module services/calibration/engine/TapeCalculator
 */

import type { TapeConfig, TapeRecommendation } from '../../../types/calibration';
import { type BedCorner, DEFAULT_TAPE_CONFIG } from '../../../types/calibration';
  ...
  getTapeInstructions(spots: TapeSpot[]): string[] {
    ...
    instructions.push(
      `Apply tape in ${spots.length} location(s)\n` +
        `Total area: ${totalArea.toFixed(1)}mmÂ²\n` +
    ...
    for (const spot of spots) {
      ...
      let instruction =
        `Position ${position}:\n` +
        `â€¢ Deviation: ${spot.heightDiff.toFixed(3)}mm\n` +
        `â€¢ Apply ${spot.layers} layer(s) of tape\n` +
        `â€¢ Area: ${spot.areaSize.toFixed(1)}mmÂ²`;
