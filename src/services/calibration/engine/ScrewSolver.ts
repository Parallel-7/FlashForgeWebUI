 ...
 *
 * @module services/calibration/engine/ScrewSolver
 */

import type { ScrewAdjustment, ScrewConfig } from '../../../types/calibration';
import { BedCorner, DEFAULT_SCREW_CONFIG, RotationDirection } from '../../../types/calibration';
...
export class ScrewSolver {
  ...
  calculateCornerAdjustment(corner: BedCorner, targetHeight: number, averagingSize = 0): ScrewAdjustment {
    ...
    if (!requiresAdjustment) {
      ...
      formattedAmount = `${Math.round(degrees)}Â° ${direction}`;
    } else {
