 ...
 *
 * This mirrors the FlashForgeUI-Electron ThumbnailCacheService, adapted for the standalone
 * WebUI: instead of Electron's userData directory it stores thumbnails under the app data
 * directory (see getDataPath()), so the same lazy-loaded file dialog cache works headlessly.
 *
 * Key Features:
 * - File-based persistence under the data directory
...
export class ThumbnailCacheService {
  ...
  private constructor(options: CacheOptions = {}) {
    // Dedicated Thumbnails directory under the app data directory
    this.basePath = options.basePath || path.join(getDataPath(), 'Thumbnails');
  }
